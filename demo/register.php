...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $err = "Username troppo corto (min 3).";
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id FROM roles WHERE name='FREE'");
            $stmt->execute();
            $freeRoleId = (int)$stmt->fetchColumn();

            $stmt = $pdo->prepare("INSERT INTO user_roles (user_id, role_id) VALUES (?,?)");
            $stmt->execute([$userId, $freeRoleId]);
...
<head>
  ...
  <title>Trademarket AI — Registrati</title>
    ...
    <section class="w-full max-w-md">
      <div class="glass rounded-2xl border border-slate-700/70 p-8 shadow-xl">
        ...
        <p class="mt-2 text-center text-sm text-slate-300">
          Crea il tuo account FREE e accedi alla piattaforma.
        </p>
        ...
        <?php if ($ok): ?>
          <div class="mt-5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-200">
            <?= htmlspecialchars($ok) ?>
          </div>
        <?php endif; ?>
        ...
        <form method="post" class="mt-6 space-y-4">
          ...
          <p class="text-center text-sm text-slate-300">
            Hai già un account?
            <a href="login.php" class="font-semibold text-primary hover:brightness-110 transition">
              Accedi
            </a>
          </p>
        </form>
